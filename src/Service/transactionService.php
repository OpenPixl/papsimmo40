...
class transactionService
{
    ...
    public function calculateProject(Transaction $transaction){
        ...
        if(is_null($transaction->isIsValidPromisepdf())){$isPromiseValid = 0;}else{$isPromiseValid = 1;}
        if(is_null($transaction->getActePdfFilename())){$isActe = 0;}else{$isActe = 1;}
        if(is_null($transaction->isIsValidActepdf())){$isActeValid = 0;}else{$isActeValid = 1;}
        if(is_null($transaction->getTracfinPdfFilename())){$isTracfin = 0;}else{$isTracfin = 1;}
        if(is_null($transaction->isIsValidtracfinPdf())){$isTracfinValid = 0;}else{$isTracfinValid = 1;}
