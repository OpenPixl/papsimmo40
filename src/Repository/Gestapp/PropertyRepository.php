...
class PropertyRepository extends ServiceEntityRepository
{
    ...
    // ----------------------------------------------
    public function StatsGraph($year)
    {
    ...
    public function listPublication()
    {
        return $this->createQueryBuilder('p')
            ...
            ->addSelect('
                ...
                pu.isPublishClefsmoi as isPublishClefsmoi,
                p.dupMandat as dupMandat,
                p.id as id,
                p.ref as ref,
                p.priceFai as priceFai,
    ...
    public function listPropertiesPublishByEmployed($user)
    {
        return $this->createQueryBuilder('p')
            ...
            ->addSelect('
                ...
                p.priceFai as priceFai,
                p.rent as rent,
                p.commerceAnnualRentGlobal as commerceAnnualRentGlobal,
                p.surfaceHome as surfaceHome,
                p.surfaceLand as surfaceLand,
                d.name as denomination,
                p.piece as piece,
    ...
    public function reportpropertycsv4($partenaire)
    {
        ...
        }elseif($partenaire == "EP"){
            $query->where('pu.isPublishEtreProprio = 1');
        }elseif($partenaire == "LBC"){
            $query->where('pu.isPublishleboncoin = 1');
        }
        $query->andWhere('p.isArchived = 0');
        $query->andWhere('p.isNomandat = 0');
        $query->select('
